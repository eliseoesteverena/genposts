// VALIDATE — una unica fuente de verdad para "es un design.json/componente/
// tokens de marca validos". Se usa en DOS lugares que antes tenian logica
// duplicada y se desincronizaron (la IA devolvio un icono "check" que el
// servidor aceptaba pero el cliente rechazaba, asi que el reintento
// automatico nunca corregia el problema):
//   1. Del lado del cliente (screen-home.js, editor.js, etc.) via window.Validate
//   2. Del lado del servidor (functions/api/generate.js) via import ES module,
//      DENTRO del loop de reintento - para que un icono invalido se corrija
//      solo antes de llegarle al usuario, no despues.

export const ICON_NAMES = ['sparkle', 'doc', 'pin', 'arrow-right', 'check', 'x', 'plus', 'star', 'heart'];
...
function validateElement(el, errors, prefix){
  ...
  if (el.type === 'icon' && el.props && ICON_NAMES.indexOf(el.props.name) === -1) {
    errors.push(prefix + ': icon.name invalido "' + (el.props && el.props.name) + '" (validos: ' + ICON_NAMES.join(', ') + ')');
  }
...
export function validateDesignDocument(doc){
  const errors = [];
...
export function validateComponentElements(elements){
  const errors = [];
...
if (typeof window !== 'undefined') {
  window.Validate = {
    validateDesignDocument: validateDesignDocument,
    validateComponentElements: validateComponentElements,
    validateBrandTokens: validateBrandTokens
  };
}
